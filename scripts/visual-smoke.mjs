...
const names = ['界面开发', '文档整理', '开发完成', '服务接口', '数据处理', '工具项目'];
const projects = names.map((name, index) => ({ id: randomUUID(), name, path: path.join(output, name), unread: index === 1 ? 1 : 0, done: index === 2, lastCompletedAt: index === 1 ? Date.now() - 60000 : null, restore: { terminal: false, codex: false } }));
...
try {
  ...
  assert.equal(await breathing(2), 0, 'manually done stays steady');
  ...
  for (const [theme, name] of [['mountain-blue', '山青蓝'], ['wild-red', '西野红']]) {
    ...
    assert.equal(await breathing(2), 0, `${name}: steady green completion`);
  }
  ...
  await fs.writeFile(path.join(output, 'results.json'), JSON.stringify({ packaged, timings, liveRange: [Math.min(...liveSamples), Math.max(...liveSamples)], compact, errors }, null, 2));
