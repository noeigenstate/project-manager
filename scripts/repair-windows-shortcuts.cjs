...
const { repairShortcuts, refreshSearchIcons } = require('../electron/windows-integration.cjs');
...
app.whenReady().then(() => {
  ...
  console.log(JSON.stringify(refreshSearchIcons({ localAppData: process.env.LOCALAPPDATA, userData: path.join(app.getPath('appData'), 'Project Grid'), iconSource: result.icon })));
