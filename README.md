...
打包后的应用位于 `release/`。双击 `Project-Grid-0.2.4-win-x64.exe` 即可启动，也可以打开 `release/win-unpacked/Project Grid.exe`。更新时，在旧版设置或托盘菜单中退出应用后再启动新版；项目列表与完成标记会保留。
...
- 总览页铺满项目网格，分类筛选和布局控件位于标题右侧，与搜索、添加项目和设置合并在同一个标题区域；窄窗口下右侧控件自动分行，无固定左栏。
...
- 图片和 HTML 文件上限为 32 MiB。文本源码支持 UTF-8 和带 BOM 的 UTF-16，上限 1 MiB；较大的 HTML 仍能渲染页面，源码可在 VS Code 中打开。
...
- 本版提供项目网格、完整终端、文件目录、图片查看、HTML 页面渲染和文本源码预览；编辑文件可通过「在 VS Code 打开」使用。
...
- `test`：状态持久化、事件传输、终端环境、目录分页、图片和 HTML 类型、资源访问边界及预览地址失效测试。
- `test:desktop`：真实终端和按钮交互、通知、目录、PNG 显示/缩放/刷新、HTML/CSS/模块脚本/JSON/图片加载、源码切换和页面隔离检查。不会向模型提交编码任务，不会修改日常工作区设置。截图在 `.test-output/desktop-*/`。添加 `-- --packaged` 可检查打包后的应用。
