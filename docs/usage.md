...
打包后的应用位于 `release/`。推荐运行 `Project-Grid-Setup-0.5.3-x64.exe` 安装，获得自动更新功能；`Project-Grid-0.5.3-win-x64.exe` 为便携版。迁移旧版时，先等任务结束，从设置或托盘退出旧版，再运行安装包。项目列表与会话历史会保留，旧人工完成项目保持停止。
...
- 点击左上方目录栏按钮或按 **Ctrl+B** 收起、展开左栏；收起后保留返回总览和设置的图标入口，终端自动适配宽度。
