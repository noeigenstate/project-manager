...
5. 点击左侧的 **返回总览**，或按 **Ctrl+Shift+G**，方框平滑缩回网格。终端进程、未发送的输入和任务继续保留；系统开启“减少动态效果”时直接切换。
...
普通、等待查看和已完成的项目均通过顶部标题栏或右上角展开按钮放大，使用同一段约 720 毫秒的渐进动画，返回约 560 毫秒。点击终端正文、输入区和底栏不会放大，可留在小窗口中输入、选字和复制；系统开启“减少动态效果”时直接切换。
...
- `node scripts/verify-installer-payload.cjs`：从实际 NSIS 安装包提取程序，核对可执行文件与应用资源；动画检查可用 `node scripts/focus-motion-smoke.mjs --executable 路径` 直接验证提取出的程序，不添加专用渲染开关。
