...
打包后的应用位于 `release/`。推荐运行 `Project-Grid-Setup-0.4.1-x64.exe` 安装，获得自动更新功能；`Project-Grid-0.4.1-win-x64.exe` 为便携版。迁移旧版时，先等任务结束，从设置或托盘退出旧版，再运行安装包。项目列表与完成标记会保留。
...
文本和 HTML 可以点击 **编辑** 后修改，按 **Ctrl+S** 或工具栏的 **保存** 写入原文件。HTML 保存后可切回「页面」查看结果。文件标签上的圆点表示未保存；切换文件、返回总览、重命名或删除当前文件、退出及安装更新前，会询问保存、放弃或取消。

保存会检测文件是否被 Codex 或其他程序修改，遇到冲突保留当前草稿并提示刷新，不直接覆盖较新的文件。大文件显示 **编辑当前段**，只替换这一段，其他字节保留；单段编辑内容上限为 1 MiB，文件总大小不受此限制。保存保留 UTF-8 / UTF-16 编码、BOM 与原有换行格式。本地与 SSH 项目均支持。
