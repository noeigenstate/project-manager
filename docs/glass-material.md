...
- **导航与控制**：顶栏、底栏和侧栏采用浮起圆角外壳，搜索框和胶囊按钮作为薄覆盖层。正文不使用普通 `filter` 或整体透明度；布局祖先不增加滤镜、变换，保持 fixed 展开/拖动坐标。
...
- **动态反馈**：边缘与圆点共用 2 秒缓进缓出周期，0 秒低亮、0.8 秒峰值、1.6 秒回落、2 秒进入下一周期。只动画装饰层的不透明度，不重绘整卡阴影或过滤终端文字。工作中持续呼吸；新的完成提醒只播放 3 次（约 6 秒），旧完成超过 9 秒的检测窗口不会被重新唤起。已查看的本轮完成保持绿色常亮；状态不再包含人工项目完成标记。焦点卡片高于悬停卡片，普通空闲卡片最弱。
- **清晰度**：项目名 14px，状态/分支/时间等信息 11–12px；窄卡片省略次要文字，保留状态点与完整悬停说明。文字保留完整不透明度和轻微暗色阴影，ANSI 调色板、用户字号、选择背景与光标不参与折射，行距为 1.3。油画原图仍为 1672×941，另外两个主题为 SVG；4K 截图验证界面高 DPI 绘制，不代表背景素材已重绘为 4K。
